...
class MealRemoteDataSourceImpl implements MealRemoteDataSource {
  ...
  Future<void> createCycle(
    ...
    await service.foodCyclesRef.doc(cycle.id).set(
          cycle.toMap(),
        );
  }
  ...
  Future<List<FoodCycle>> getAllCycles() async {
    final snapshot = await service.foodCyclesRef
        .where(
          "isDeleted",
          isEqualTo: false,
        )
  ...
  Future<void> updateCycle(
    ...
    await service.foodCyclesRef.doc(cycle.id).update(
          cycle
              .copyWith(
  ...
  Future<void> deleteCycle(
    ...
    await service.foodCyclesRef.doc(cycleId).update({
      "isDeleted": true,
...
// =========================
  ...
  Future<void> updateCycleStats(
    ...
    final cycleDoc = await service.foodCyclesRef.doc(cycleId).get();
    ...
    final remaining = cycle.totalTiffin - totalEaten;
    ...
    await service.foodCyclesRef.doc(cycleId).update({
      "totalEaten": totalEaten,
      "remainingTiffin": remaining < 0 ? 0 : remaining,
      "status": status.name,
      "updatedAt": Timestamp.now(),
    });
  }
