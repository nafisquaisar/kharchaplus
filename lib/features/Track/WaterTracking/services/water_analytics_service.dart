...
class WaterAnalyticsService {
  ...
  WaterIntakeAnalytics calculateWaterIntakeAnalytics({
	...
	final averageDaily = monthlyTotal == 0
		? 0
		: (monthlyTotal / daysElapsed).round();
  ...
  ExpenseAnalytics calculateExpenseAnalytics({
	...
	for (final item in purchases) {
	  ...
	  } else if (!item.date.isBefore(prevStart) && item.date.isBefore(prevEnd)) {
		previousExpense += item.price;
	  }
	}

	final averageCost = totalPurchases == 0
		? 0.0
		: monthlyExpense / totalPurchases;
...
class ExpenseAnalytics {
  ...
  final Map<String, int> purchaseCountByType;
  final Map<String, double> purchaseCostByType;
